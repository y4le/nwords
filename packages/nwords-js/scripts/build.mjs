...
if (process.argv.slice(2).some(arg => arg !== '--dev')) throw new Error('Usage: npm run build -- [--dev]');
...
// No unpinned wasm-opt download. Release optimization is performed by pinned rustc.
run('wasm-pack', ['build', 'crates/nwords-js', '--target', 'web', '--release', '--no-opt', '--no-pack', '--out-dir', output + '/wasm', '--locked']);
...
const provenance = {
  ...
  wasm: { target: 'web', optimization: 'rustc-release; wasm-opt disabled', bytes: wasm.length, sha256: sha256(wasm) },
