...
run('wasm-pack', ['build', 'crates/nwords-js-variable', '--target', 'web', ...profileArgs, '--no-opt', '--no-pack', '--out-dir', output + '/wasm/variable', '--locked']);
if (optimizerInfo) {
  for (const asset of ['wasm/nwords_js_bg.wasm', 'wasm/bip39/nwords_js_bip39_bg.wasm', 'wasm/variable/nwords_js_variable_bg.wasm']) {
...
await rm(join(output, 'wasm/variable/.gitignore'));
...
const graph = new Set(['nwords-js', 'nwords-js-bip39', 'nwords-js-variable'].flatMap(name =>
...
const variableWasm = await readFile(join(output, 'wasm/variable/nwords_js_variable_bg.wasm'));
const provenance = {
  ...
  variableWasm: { target: 'web', optimization: `rustc-${profile === 'baseline' ? 'release' : 'wasm-' + profile}; ${optimizerInfo ? optimizerInfo.version + ' -O3' : 'wasm-opt disabled'}`, profile, optimizer: optimizerInfo ?? null, bytes: variableWasm.length, sha256: sha256(variableWasm) },
