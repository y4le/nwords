...
try {
  ...
  const noRandomPage = await browser.newPage();
  await noRandomPage.addInitScript(() => {
    Object.defineProperty(window.crypto, 'getRandomValues', { value() { throw new Error('unavailable'); } });
  });
  await noRandomPage.goto(origin + '/demo/');
  await noRandomPage.click('#bitcoin-tab');
  assert.equal(await noRandomPage.inputValue('#entropy-value'), '');
  assert.match(await noRandomPage.locator('#bitcoin-status').textContent(), /Secure randomness unavailable/);
  await noRandomPage.fill('#entropy-value', typedEntropy);
  await noRandomPage.waitForFunction(expected => document.querySelector('#mnemonic-value')?.value === expected, typedMnemonic);
  await noRandomPage.close();
  ...
  const initialEntropy = await demo.inputValue('#entropy-value');
  assert.match(initialEntropy, /^[0-9a-f]{32}$/, 'Bitcoin demo starts with 128 bits of entropy');
  assert.match(await demo.locator('#bitcoin-code').textContent(), /crypto\.getRandomValues\(new Uint8Array\(16\)\)/);
  ...
  await demo.waitForFunction(() => document.querySelector('#bitcoin-status')?.textContent === 'Round trip ready');
  const initialMnemonic = await demo.inputValue('#mnemonic-value');
  assert.equal(initialMnemonic.split(' ').length, 12);
  await demo.fill('#entropy-value', '');
  await demo.fill('#mnemonic-value', initialMnemonic);
  assert.equal(await demo.inputValue('#entropy-value'), initialEntropy);
  ...
  const secondVisit = await browser.newPage();
  await secondVisit.goto(origin + '/demo/');
  const freshEntropy = await secondVisit.inputValue('#entropy-value');
  assert.match(freshEntropy, /^[0-9a-f]{32}$/);
  assert.notEqual(freshEntropy, initialEntropy, 'A new page load starts with fresh entropy');
